...
const handlePullAndStageTiles = (state, action) => {
  return produce(state, draft => {
    ...
    if (factoryIndex === -1) {
      ...
      if (draft.firstPlayerNextRound === null) {
        // If first player to pull from table, give them the first player token
        const firstAvailableIndex = board.brokenTiles.indexOf(null)
        if (firstAvailableIndex !== -1) {
          board.brokenTiles[firstAvailableIndex] = FIRST_PLAYER_TOKEN
        }
        draft.firstPlayerNextRound = playerIndex
      }
